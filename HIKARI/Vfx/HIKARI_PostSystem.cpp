...
namespace HIKARI {
    namespace POST {
        ...
        namespace {
            ...
            static bool HasSlotOverrideValue(const DirectX::XMFLOAT4& v) {
                return !IsNearZero(v.x) || !IsNearZero(v.y) || !IsNearZero(v.z) || !IsNearZero(v.w);
            }
        ...
        bool PostSystem::SetGlobalProfile(const std::string& profileId, const DirectX::XMFLOAT4(&userOverrides)[16]) {
            ...
            if (HasAnyOverrideValue(userOverrides)) {
                for (size_t i = 0; i < activeGlobalProfile_.values.size(); ++i) {
                    if (HasSlotOverrideValue(userOverrides[i])) {
