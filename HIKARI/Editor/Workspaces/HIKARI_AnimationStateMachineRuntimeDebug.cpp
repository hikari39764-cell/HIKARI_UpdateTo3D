...
namespace HIKARI::EDITOR {
    ...
    const ANIMATION::AnimationStateMachineRuntimeSnapshot*
        AnimationStateMachineWorkspaceController::
            ResolveRuntimeDebugSnapshot(DocumentSceneBase& scene) {
        if (!document_.GetAssetGuid().IsValid()) return nullptr;
        auto* service = scene.GetWorld().Services().Find<
        ...
        const auto findForObject = [this, service](GameObject* object)
            ...
            const auto* component = object->GetComponent<
                AnimationStateMachineComponent>();
            if (component == nullptr || component->GetAssetGuid() !=
                    document_.GetAssetGuid().value) {
                return nullptr;
            }
            const auto* instance = service->Find(
                object->GetRuntimeHandle());
            return instance != nullptr ? &instance->GetSnapshot() : nullptr;
        };

        if (const auto* selected = findForObject(
    ...
    void AnimationStateMachineWorkspaceController::DrawRuntimeDebug(
        ...
        if (ImGui::BeginCombo("Runtime Object", currentName.c_str())) {
            for (const auto& object : scene.GetWorld().GetObjects()) {
                ...
                if (component == nullptr || instance == nullptr ||
                    component->GetAssetGuid() !=
                        document_.GetAssetGuid().value) {
                    continue;
                }
                const bool selected = object->GetRuntimeHandle() ==
                    runtimeDebugObject_;
                const std::string label = object->GetName() + "##Runtime" +
        ...
        if (snapshot == nullptr) {
            ImGui::TextDisabled(
                "No playing object currently uses this saved asset.");
            return;
        }
        ...
        if (snapshot->motionType ==
            ...
            ImGui::Text(
                ...
                snapshot->blendLowerThreshold,
                snapshot->blendUpperThreshold);
            ImGui::Text("Secondary: %s",
                snapshot->secondaryClipName.empty()
                    ? "<none>"
                    : snapshot->secondaryClipName.c_str());
            ImGui::ProgressBar(
                snapshot->motionBlendWeight,
                { -1.0f, 0.0f },
                "Motion Blend");
        }
        ImGui::Text("Effective Speed: %.3f", snapshot->effectiveSpeed);
        const auto* instance = service->Find(runtimeDebugObject_);
        const auto* asset = instance != nullptr
            ? instance->GetAsset()
            : nullptr;
        if (asset != nullptr && ImGui::CollapsingHeader(
                "Parameters", ImGuiTreeNodeFlags_DefaultOpen)) {
            ...
            if (ImGui::BeginTable("RuntimeParameters", 2, flags)) {
                ...
                for (const auto& parameter :
                    ...
                    if (const bool* boolean = value != nullptr
                            ? std::get_if<bool>(value)
                            : nullptr) {
                        ImGui::TextUnformatted(*boolean ? "True" : "False");
                    } else if (const float* number = value != nullptr
