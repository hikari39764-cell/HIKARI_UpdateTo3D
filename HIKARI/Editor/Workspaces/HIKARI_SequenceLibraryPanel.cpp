...
namespace HIKARI::EDITOR {
    ...
    namespace {
        ...
        std::string ShortGuid(const AssetGuid& guid) {
            return guid.value.size() <= 8
                ? guid.value
                : guid.value.substr(0, 8);
        }
    }

    SequenceLibraryPanelResult SequenceLibraryPanel::Draw(
        ...
        ImGui::SameLine();
        ImGui::TextDisabled("%d assets", static_cast<int>(
            assetDatabase.CollectByType(AssetType::Sequence).size()));
        ImGui::SetNextItemWidth(-1.0f);
        ImGui::InputTextWithHint(
            "##SequenceLibrarySearch",
        ...
        const bool canOpen = actionsAllowed &&
            selectedAssetGuid_.IsValid();
        if (!canOpen) {
            ImGui::BeginDisabled();
        }
