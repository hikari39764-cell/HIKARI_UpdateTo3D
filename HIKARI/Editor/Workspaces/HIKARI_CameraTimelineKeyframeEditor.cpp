...
namespace HIKARI::EDITOR {
    ...
    CameraTimelineKeyframeEditorResult CameraTimelineKeyframeEditor::Draw(
        ...
        if (marqueeActive_) {
            const ImVec2 minimum{
                (std::min)(marqueeStartX_, mousePosition.x),
                (std::min)(marqueeStartY_, mousePosition.y)
            };
            const ImVec2 maximum{
                (std::max)(marqueeStartX_, mousePosition.x),
                (std::max)(marqueeStartY_, mousePosition.y)
            };
            drawList.AddRectFilled(
                minimum,
                maximum,
                IM_COL32(76, 139, 211, 42));
            drawList.AddRect(
                minimum,
                maximum,
                IM_COL32(105, 173, 245, 220),
                0.0f,
                0,
                1.5f);
    ...
    bool CameraTimelineKeyframeEditor::SelectAll(
        CinematicSequence& sequence) {
