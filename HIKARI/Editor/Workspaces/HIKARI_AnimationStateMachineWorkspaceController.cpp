...
namespace HIKARI::EDITOR {
    ...
    bool AnimationStateMachineWorkspaceController::IsDocumentOpen()
        const noexcept {
    ...
    bool AnimationStateMachineWorkspaceController::CanUndo()
        const noexcept {
        return document_.CanUndo();
    }

    bool AnimationStateMachineWorkspaceController::CanRedo()
        const noexcept {
        return document_.CanRedo();
    }

    bool AnimationStateMachineWorkspaceController::Save(
        DocumentSceneBase& scene,
        std::string& outMessage) {
        const bool saved = document_.Save(
            scene.GetAssetDatabase(), outMessage);
        if (saved) {
            if (auto* store = scene.GetWorld().Services().Find<
    ...
    bool AnimationStateMachineWorkspaceController::Undo(
        std::string& outMessage) {
        if (!document_.Undo()) {
    ...
    bool AnimationStateMachineWorkspaceController::Redo(
        std::string& outMessage) {
        if (!document_.Redo()) {
    ...
    void AnimationStateMachineWorkspaceController::AddState() {
        ...
        AddStateAt(
            100.0f +
                static_cast<float>(definition.states.size() % 4u) * 190.0f,
            100.0f +
                static_cast<float>(definition.states.size() / 4u) * 120.0f);
    }

    void AnimationStateMachineWorkspaceController::AddStateAt(
        float graphX,
        float graphY) {
    ...
    void AnimationStateMachineWorkspaceController::DeleteSelectedState() {
        auto& definition = document_.Definition();
        if (definition.states.size() <= 1u ||
            !selectedStateId_.IsValid()) {
        ...
        definition.states.erase(
            std::remove_if(
                definition.states.begin(), definition.states.end(),
    ...
    void AnimationStateMachineWorkspaceController::AddParameter(
        ...
        parameter.defaultValue =
            ANIMATION::MakeDefaultAnimationParameterValue(type);
    ...
    void AnimationStateMachineWorkspaceController::DeleteSelectedParameter() {
        if (!selectedParameterId_.IsValid()) return;
        auto before = document_.Definition();
        auto& definition = document_.Definition();
    ...
    void AnimationStateMachineWorkspaceController::DeleteSelectedTransition() {
        if (!selectedTransitionId_.IsValid()) return;
        auto before = document_.Definition();
    ...
    void AnimationStateMachineWorkspaceController::NormalizeSelection()
        noexcept {
        const auto& definition = document_.Definition();
        if (selectionKind_ == SelectionKind::State &&
            ANIMATION::FindAnimationState(definition, selectedStateId_) ==
                nullptr) {
            selectedStateId_ = definition.entryStateId;
        }
        if (selectionKind_ == SelectionKind::Parameter &&
            ANIMATION::FindAnimationParameter(
                definition, selectedParameterId_) == nullptr) {
            selectionKind_ = SelectionKind::None;
            selectedParameterId_ = {};
        }
    ...
    ANIMATION::AnimationState*
        ...
        return ANIMATION::FindAnimationState(
            document_.Definition(), selectedStateId_);
    }

    ANIMATION::AnimationParameterDefinition*
        AnimationStateMachineWorkspaceController::SelectedParameter()
            noexcept {
        return ANIMATION::FindAnimationParameter(
            document_.Definition(), selectedParameterId_);
    }

    ANIMATION::AnimationStateTransition*
        AnimationStateMachineWorkspaceController::SelectedTransition()
            noexcept {
        auto& transitions = document_.Definition().transitions;
        const auto found = std::find_if(
            transitions.begin(), transitions.end(),
