...
namespace HIKARI::EDITOR {
    ...
    namespace {
        ...
        void MoveTransformSelections(
            SEQUENCER::CameraTransformTrack& track,
            const Items& items,
            float deltaSeconds) {
            ...
            for (SEQUENCER::CameraTransformChannel& channel : track.channels) {
                ...
                channel.keyframes.erase(
                    ...
                    channel.keyframes.end());
                for (SEQUENCER::CameraTransformKeyframe& keyframe :
                        channel.keyframes) {
                    ...
                    if (found != items.end()) {
                        keyframe.timeSeconds =
                            found->startTimeSeconds + deltaSeconds;
                    }
                }
            }
        ...
        template <typename Items>
        void MoveLensSelections(
            SEQUENCER::CameraLensTrack& track,
            const Items& items,
            float deltaSeconds) {
            ...
            for (SEQUENCER::CameraLensChannel& channel : track.channels) {
                ...
                channel.keyframes.erase(
                    ...
                    channel.keyframes.end());
                for (SEQUENCER::CameraLensKeyframe& keyframe :
                        channel.keyframes) {
                    const auto found = std::find_if(
                        items.begin(),
                        items.end(),
    ...
    bool CameraTimelineKeyframeDragSession::Update(
        ...
        if (snapEnabled && snapFramesPerSecond > 0.0f &&
            primary != items_.end()) {
            const float primaryTime =
                primary->startTimeSeconds + deltaSeconds;
            deltaSeconds =
                std::round(primaryTime * snapFramesPerSecond) /
                    snapFramesPerSecond -
        ...
        if (std::abs(deltaSeconds - lastDeltaSeconds_) <=
            kKeyframeTimeEpsilon) {
            return false;
        }
