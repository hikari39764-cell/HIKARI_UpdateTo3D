...
namespace HIKARI::EDITOR {
    namespace {
        ...
        ModelAsset* ResolvePreviewModel(
            ...
            if (definition.previewModelAssetId.value.empty()) return nullptr;
            ModelAsset* model = scene.GetModelManager().FindAsset(
                definition.previewModelAssetId.value);
            if (model == nullptr ||
                model->GetState() != ModelAsset::State::Loaded) {
                (void)scene.GetModelManager().LoadAssetNow(
                    definition.previewModelAssetId.value);
                model = scene.GetModelManager().FindAsset(
                    definition.previewModelAssetId.value);
            }
        ...
        bool DrawClipPicker(
            ...
            if (ImGui::BeginCombo(label, preview)) {
                for (size_t index = 0u;
                        index < model.animations.size(); ++index) {
                    const AnimationClip& clip = model.animations[index];
                    const bool selected = model.GetAnimationClipId(index) ==
                        reference.clipId;
        ...
        const char* MotionTypeName(
            ANIMATION::AnimationStateMotionType type) noexcept {
    ...
    void AnimationStateMachineWorkspaceController::
        DrawSelectedStateMotionEditor(
            DocumentSceneBase& scene,
        ...
        if (ImGui::BeginCombo("Motion Type", MotionTypeName(motionType))) {
            ...
            for (const auto candidate : types) {
                ...
                if (ImGui::Selectable(
                        MotionTypeName(candidate), selected) && !selected) {
        ...
        if (auto* clipMotion = std::get_if<
                ANIMATION::AnimationClipMotion>(&state.motion)) {
            if (model != nullptr && !model->animations.empty()) {
                auto before = definition;
                if (DrawClipPicker(
                        "Animation Clip", *model, clipMotion->clip)) {
                    RecordMutation(std::move(before));
                }
            }
            return;
        ...
        const ANIMATION::AnimationParameterDefinition* selectedParameter =
            ANIMATION::FindAnimationParameter(
                definition, blendTree->parameterId);
        ...
        if (ImGui::BeginCombo("Blend Parameter", parameterPreview)) {
            for (const auto& parameter : definition.parameters) {
                ...
                const bool selected = parameter.id ==
        ...
        if (selectedParameter == nullptr) {
            if (ImGui::Button("Create Float Parameter")) {
                ...
                parameter.id = ANIMATION::AllocateAnimationParameterId(
                    definition);
        ...
        if (ImGui::Button("+ Blend Sample")) {
            ...
            if (model != nullptr && !model->animations.empty()) {
                sample.clip = ANIMATION::MakeAnimationClipReference(
                    *model, 0u);
            }
        ...
        if (ImGui::Button("Evenly Space")) {
            ...
            if (last <= first) {
                last = first + static_cast<float>(
                    blendTree->samples.size() - 1u);
            }
            const float step = (last - first) /
                static_cast<float>(blendTree->samples.size() - 1u);
            for (size_t index = 0u;
        ...
        if (!ImGui::BeginTable("BlendSamples", 4, tableFlags)) return;
        ImGui::TableSetupColumn("Threshold", ImGuiTableColumnFlags_WidthFixed,
            90.0f);
        ...
        for (size_t index = 0u; index < blendTree->samples.size(); ++index) {
            ...
            auto before = definition;
            if (ImGui::DragFloat(
                    "##Threshold", &sample.threshold, 0.02f)) {
                RecordMutation(
                    std::move(before),
                    12000u + state.id.value * 128u + index);
                ImGui::PopID();
                ImGui::EndTable();
                return;
            }
            ...
            before = definition;
            if (ImGui::DragFloat(
                    "##Speed", &sample.speedScale, 0.02f, -8.0f, 8.0f)) {
                RecordMutation(
                    std::move(before),
                    14000u + state.id.value * 128u + index);
                ImGui::PopID();
                ImGui::EndTable();
                return;
            }
            ...
            if (ImGui::SmallButton("X")) {
                before = definition;
                blendTree->samples.erase(
                    blendTree->samples.begin() + index);
                RecordMutation(std::move(before));
                ImGui::PopID();
                ImGui::EndTable();
                return;
