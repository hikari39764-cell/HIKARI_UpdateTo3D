...
namespace HIKARI::EDITOR {
    ...
    bool CameraTimelineSelectionSet::Contains(
        ...
        return std::find(items_.begin(), items_.end(), selection) !=
            items_.end();
    }
    ...
    bool CameraTimelineSelectionSet::Apply(
        ...
        if (!selection.IsValid()) {
            return operation == CameraTimelineSelectionOperation::Replace
                ? Clear()
                : false;
        }
        return Apply(
            std::vector<CameraTimelineKeyframeSelection>{ selection },
            operation);
    }
    ...
    bool CameraTimelineSelectionSet::Apply(
        ...
        if (operation == CameraTimelineSelectionOperation::Replace) {
            ...
            for (const CameraTimelineKeyframeSelection& selection :
                    selections) {
                if (selection.IsValid() &&
                    std::find(unique.begin(), unique.end(), selection) ==
                        unique.end()) {
