...
namespace HIKARI::EDITOR {
    namespace {
        ...
        const char* ParameterSourceName(
            ANIMATION::AnimationParameterSource source) noexcept {
            using Source = ANIMATION::AnimationParameterSource;
            switch (source) {
            case Source::CharacterGrounded: return "Character Grounded";
            case Source::CharacterMoving: return "Character Moving";
            case Source::CharacterHorizontalSpeed: return "Horizontal Speed";
            case Source::CharacterVerticalSpeed: return "Vertical Speed";
            case Source::CharacterInputMagnitude: return "Input Magnitude";
            case Source::CharacterSprinting: return "Character Sprinting";
            case Source::CharacterFalling: return "Character Falling";
            case Source::Manual:
            default: return "Manual (C++)";
            }
        }

        ANIMATION::AnimationConditionOperator DefaultOperator(
            ...
            if (type == Type::Trigger) return Op::Triggered;
            if (type == Type::Bool) return Op::IsTrue;
            return Op::Greater;
        }

        const char* ConditionOperatorName(
            ANIMATION::AnimationConditionOperator value) noexcept {
            using Op = ANIMATION::AnimationConditionOperator;
            switch (value) {
            case Op::IsFalse: return "Is False";
            ...
            case Op::Triggered: return "Triggered";
            case Op::IsTrue:
    ...
    void AnimationStateMachineWorkspaceController::DrawDetailsWindow(
        ...
        if (!definition.previewModelAssetId.value.empty()) {
            const AssetRecord* selectedRecord =
                scene.GetAssetDatabase().FindByGuid(
                    AssetGuid{ definition.previewModelAssetId.value });
            modelPreview = selectedRecord != nullptr &&
                    !selectedRecord->displayName.empty()
        ...
        if (ImGui::BeginCombo("Animation Model", modelPreview.c_str())) {
            for (const AssetRecord* record : models) {
                ...
                const bool selected = record->guid.value ==
                    definition.previewModelAssetId.value;
                const std::string label = record->displayName + "##Model" +
                    record->guid.value;
        ...
        if (selectionKind_ == SelectionKind::State) {
            ...
            if (state != nullptr) {
                ...
                if (InputTextString("State Name", state->name)) {
                    RecordMutation(
                        std::move(before), 1000u + state->id.value);
                    state = SelectedState();
                }
                ...
                before = definition;
                if (ImGui::DragFloat(
                        "Speed", &state->speed, 0.02f, -8.0f, 8.0f)) {
                    RecordMutation(
                        std::move(before), 2000u + state->id.value);
                    state = SelectedState();
                }
                before = definition;
            ...
            if (parameter != nullptr) {
                ...
                constexpr const char* sources[]{
                    "Manual (C++)", "Character Grounded",
                    "Character Moving", "Horizontal Speed",
                    "Vertical Speed", "Input Magnitude",
                ...
                if (ImGui::Combo("Binding Source", &source, sources, 8)) {
                    parameter->source = static_cast<
                        ANIMATION::AnimationParameterSource>(source);
                    RecordMutation(std::move(before));
                    parameter = SelectedParameter();
                }
                ImGui::EndDisabled();
                ImGui::TextDisabled(
                    "Source: %s", ParameterSourceName(parameter->source));

                before = definition;
                if (parameter->type ==
                        ANIMATION::AnimationParameterType::Float) {
                    float value = std::get<float>(parameter->defaultValue);
                    if (ImGui::DragFloat("Default", &value, 0.02f)) {
                        parameter->defaultValue = value;
                        RecordMutation(
                            std::move(before), 4000u + parameter->id.value);
                    }
                    ...
                    if (ImGui::DragInt("Default", &value)) {
                        ...
                        ANIMATION::AnimationParameterType::Bool) {
            ...
            if (transition != nullptr) {
                ...
                if (ImGui::BeginCombo(
                    ...
                    const bool anySelected =
                        !transition->sourceStateId.IsValid();
                    ...
                    for (const auto& state : definition.states) {
                        const bool selected =
                            state.id == transition->sourceStateId;
                        if (ImGui::Selectable(
                                state.name.c_str(), selected)) {
                            before = definition;
                ...
                if (ImGui::BeginCombo(
                    ...
                    for (const auto& state : definition.states) {
                        const bool selected =
                            state.id == transition->targetStateId;
                        if (ImGui::Selectable(
                                state.name.c_str(), selected)) {
                            before = definition;
                ...
                before = definition;
                if (ImGui::DragInt(
                        "Priority", &transition->priority, 1.0f)) {
                    RecordMutation(
                        std::move(before), 5000u + transition->id.value);
                    transition = SelectedTransition();
                }
                before = definition;
                if (ImGui::DragFloat(
                        ...
                        "%.2f s")) {
                    RecordMutation(
                        std::move(before), 6000u + transition->id.value);
                    transition = SelectedTransition();
                }
                before = definition;
                if (ImGui::Checkbox(
                        "Require Exit Time",
                        &transition->requireExitTime)) {
                    RecordMutation(std::move(before));
                    transition = SelectedTransition();
                }
                ...
                if (ImGui::Button("+ Condition")) {
                    ...
                    transition->conditions.push_back({
                        parameter.id,
                        DefaultOperator(parameter.type),
                        ANIMATION::MakeDefaultAnimationParameterValue(
                            parameter.type)
                    });
                    RecordMutation(std::move(before));
                ...
                ImGui::EndDisabled();
                for (size_t index = 0u;
                    index < transition->conditions.size();) {
                    ...
                    const char* parameterPreview = parameter != nullptr
                        ? parameter->name.c_str()
                        : "<missing>";
                    if (ImGui::BeginCombo("Parameter", parameterPreview)) {
                        for (const auto& candidate : definition.parameters) {
                            const bool selected =
                                candidate.id == condition.parameterId;
                            if (ImGui::Selectable(
                                    candidate.name.c_str(), selected)) {
                                before = definition;
                                condition.parameterId = candidate.id;
                                condition.comparison =
                                    DefaultOperator(candidate.type);
                                condition.threshold =
                                    ANIMATION::
                                        MakeDefaultAnimationParameterValue(
                                            candidate.type);
                                RecordMutation(std::move(before));
                                transition = SelectedTransition();
                                parameter =
                                    ANIMATION::FindAnimationParameter(
                                        definition,
                    ...
                    if (parameter != nullptr) {
                        using Type = ANIMATION::AnimationParameterType;
                        using Op = ANIMATION::AnimationConditionOperator;
                        const std::array<Op, 6> numericOps{
                            Op::Greater, Op::GreaterOrEqual, Op::Less,
                            Op::LessOrEqual, Op::Equal, Op::NotEqual
                        };
                        const std::array<Op, 4> boolOps{
                            Op::IsTrue, Op::IsFalse, Op::Equal, Op::NotEqual
                        };
                        ...
                        if (parameter->type == Type::Float) {
                            float value =
                                std::get<float>(condition.threshold);
                            if (ImGui::DragFloat(
                                    "Threshold", &value, 0.02f)) {
                                before = definition;
                            ...
                            int value =
                                std::get<int32_t>(condition.threshold);
                            ...
                            bool value =
                                std::get<bool>(condition.threshold);
                    ...
                    if (ImGui::SmallButton("X")) {
                        ...
                        transition->conditions.erase(
                            transition->conditions.begin() + index);
                        RecordMutation(std::move(before));
                        transition = SelectedTransition();
                        removed = true;
                    }
            ...
            ImGui::TextDisabled(
                "Select a state, parameter, or transition to edit it.");
        }
        ImGui::End();
#else
        (void)scene;
